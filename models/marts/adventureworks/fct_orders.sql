with 
    ...
    , joined as (
        ...
        from sales_agregate
        left join product on product.sales_order_id = sales_agregate.sales_order_id
        left join localization on localization.address_id = sales_agregate.ship_to_address_id
        left join customer on customer.customer_id = sales_agregate.customer_id
    )  
    , fct_orders as (
        select 
